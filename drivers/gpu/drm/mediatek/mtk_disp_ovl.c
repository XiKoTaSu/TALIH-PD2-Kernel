...
//Begin by yongzhuo for LUSHAN-146 on 2023.07.31
#ifdef CONFIG_TCT_DISPLAY_SWAP_DSI0_DSI1
		offset = (src_x + dst_w) * drm_format_plane_cpp(fmt, 0) + src_y * pitch;
#else
        offset = src_x * drm_format_plane_cpp(fmt, 0) + src_y * pitch;
#endif
//End by yongzhuo for LUSHAN-146 on 2023.07.31
	addr += offset;
...
//Begin by yongzhuo for LUSHAN-146 on 2023.07.31
		...
		_get_bg_roi(comp, &bg_h, &bg_w);
		offset = (pending->dst_y << 16) | (bg_w - pending->width - pending->dst_x);
		con |= CON_HORI_FLIP;
#else
