...
int charger_dev_mode(struct charger_device *chg_dev, u32 mode)
{
	if (chg_dev != NULL && chg_dev->ops != NULL && chg_dev->ops->set_cp_mode)
		return chg_dev->ops->set_cp_mode(chg_dev, mode);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_mode);
#endif
