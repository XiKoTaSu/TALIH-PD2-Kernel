...
/* Parameters */
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER)
#define DV2_VTA_INIT		5500	/* mV */
#define DV2_ITA_INIT		2000	/* mA */
#else
...
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER)
#define DV2_DVCHG_STARTUP_CONVERT_RATIO		213	/* % */
#else
...
struct dv2_algo_desc {
	...
	u32 ita_level_save[DV2_RCABLE_MAX];	/* input current */
#endif
...
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER) && IS_ENABLED(CONFIG_MTK_PD_POLICY_MODE)
/*
 * mode divider charger
 *
 * @en: mode set
 */
static int __dv2_set_mode_dvchg_charging(struct dv2_algo_info *info,
				       enum dv2_dvchg_role role, u32 mode)
{
	int ret;
	struct dv2_algo_data *data = info->data;
	...
	PCA_INFO("mode[%s] = %d\n", __dv2_dvchg_role_name[role], mode);
	ret = prop_chgalgo_set_mode(data->pca_dvchg[role], mode);
	if (ret < 0) {
		PCA_ERR("mode set fail(%d)\n", ret);
		return ret;
	}
	return 0;
}
...
cc_cv:
	...
	if (data->ita_measure + ita_gap_per_vstep > idvchg_lmt ||
	    vta == auth_data->vcap_max){
		data->state = DV2_ALGO_CC_CV;
		PCA_ERR("will chang to CV(%d:%d:%d) (%d:%d) step(%d:%d)\n",data->ita_measure,ita_gap_per_vstep,idvchg_lmt, vta,auth_data->vcap_max,
...
cc_cv:
	...
	if (vbat >= data->vbat_cv) {
		...
		vta -= auth_data->vta_step;
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER)
		if( (data->ita_measure - ita) >= (ita_gap_per_vstep/2))
			ita -= 0;
		else
			ita -= ita_gap_per_vstep;
#else
...
static bool __dv2_is_ta_rdy(struct dv2_algo_info *info)
{
	...
	if (data->ta_ready)
		return true;
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER) && IS_ENABLED(CONFIG_MTK_PD_POLICY_MODE)
	for(val=0; val<DV2_RCABLE_MAX; val++){
		desc->ita_level[val] = desc->ita_level_save[val];
	}
#endif
...
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER) && IS_ENABLED(CONFIG_MTK_PD_POLICY_MODE)
		if( (auth_data->vta_max) >= 20000){/*default 4:1 mode*/
			data->vta_mode = 0;
			__dv2_set_mode_dvchg_charging(info, DV2_DVCHG_MASTER, data->vta_mode);
		}
		else{
			data->vta_mode = 1;/*2:1*/
			__dv2_set_mode_dvchg_charging(info, DV2_DVCHG_MASTER, data->vta_mode);
			data->idvchg_term = data->idvchg_term*2;
			auth_data->vcap_max = 11000;/*11v*/
			for(val=0; val<DV2_RCABLE_MAX; val++){
				desc->ita_level[val] = desc->ita_level[val]*2;
			}
		}
		PCA_DBG("vta_mode=%d\n",data->vta_mode);
#endif
...
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER) && IS_ENABLED(CONFIG_MTK_PD_POLICY_MODE)
	for(i=0;i <DV2_RCABLE_MAX;i++){
		desc->ita_level_save[i] = desc->ita_level[i];
	}
#endif
	return 0;
}
