...
void sw_jeita_state_machine_init(struct charger_manager *info)
{
	...
	if (battery_psy != NULL) {
		power_supply_get_property(battery_psy,	POWER_SUPPLY_PROP_TEMP,&propval);
		info->battery_temp = propval.intval;
		printk("[SW_JEITA] battery_temp:%d intval:%d \n",info->battery_temp, propval.intval);
	} else {
		printk("power_supply_get_by_name failed \n");
		info->battery_temp = 10 * battery_get_bat_temperature();
	}
#endif
...
static void charger_update_data(struct charger_manager *info)
{
	...
	if (battery_psy != NULL) {
		power_supply_get_property(battery_psy,	POWER_SUPPLY_PROP_TEMP, &propval);
		info->battery_temp = propval.intval;
		printk("[SW_JEITA] battery_temp:%d intval:%d gettemp:%d\n",info->battery_temp, propval.intval, battery_get_bat_temperature());
	} else {
		printk("power_supply_get_by_name failed \n");
		info->battery_temp = 10 * battery_get_bat_temperature();
	}
