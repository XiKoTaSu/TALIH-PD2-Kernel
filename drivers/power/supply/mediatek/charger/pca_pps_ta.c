				   ...
				   struct prop_chgalgo_ta_auth_data *data)
{
	...
	while (1) {
		...
		if( (apdo_cap.max_mv) >= 20000){/*4:1 mode*/
			data->vcap_max = 20000;/*20v*/
		}
		else{/*2:1 mode*/
			data->vcap_max = 11000;/*11v*/
		}
#endif
		/*
		...
		if (apdo_idx == -1 || apdo_cap.ma > selected_apdo_cap.ma 
		    || ((apdo_cap.max_mv >selected_apdo_cap.ma) && (apdo_cap.ma > 2000)) ) {
#else
		if (apdo_idx == -1 || apdo_cap.ma > selected_apdo_cap.ma) {
#endif
...
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MANAGER)
		if( (apdo_cap.min_mv > 5000) || (apdo_cap.ma < 3000) )
			ret = pca_pps_enable_charging(pca, true, 5500, apdo_cap.ma);
		else
			ret = pca_pps_enable_charging(pca, true, 5000, 3000);
#if IS_ENABLED(CONFIG_MTK_PD_POLICY_MODE)
		if( (data->vta_max) >= 20000){
			data->ita_gap_per_vstep = 100;/*4:1 mode*/
		}else{
			data->ita_gap_per_vstep = 200;/*2:1 mode*/
		}
#endif/*end CONFIG_MTK_PD_POLICY_MODE*/
#else
		ret = pca_pps_enable_charging(pca, true, 5000, 3000);
#endif
		if (ret != TCP_DPM_RET_SUCCESS)
...
{
	return platform_driver_unregister(&pca_pps_platdrv);
}
