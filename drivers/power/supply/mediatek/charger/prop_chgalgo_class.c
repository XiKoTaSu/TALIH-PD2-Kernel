...
int prop_chgalgo_set_mode(struct prop_chgalgo_device *pca, u32 mode)
{
	if (pca_check_devtype(pca, PCA_DEVTYPE_CHARGER) < 0)
		return -EINVAL;
	if (!pca->desc->chg_ops->set_mode)
		return -ENOTSUPP;
	return pca->desc->chg_ops->set_mode(pca, mode);
}
EXPORT_SYMBOL(prop_chgalgo_set_mode);
#endif
