...
struct ic_setup_collect g_hx83121a_setup = {
	...
	._addr_rawout_sel = HX83121A_ADDR_RAWOUT_SEL,
...
void hx83121a_burst_enable(uint8_t auto_add_4_byte)
{
	...
	hx_parse_assign_cmd(HX83121A_PARA_AHB_INC4, tmp_data, DATA_LEN_1);
...
void hx83121a_interface_on(void)
{
	uint8_t tmp_addr[DATA_LEN_4];
	uint8_t tmp_data[DATA_LEN_4];
	...
	do {
		...
		hx_parse_assign_cmd(ADDR_AHB_INC4, tmp_addr, DATA_LEN_1);
		hx_parse_assign_cmd(HX83121A_PARA_AHB_INC4,
			tmp_data,
			DATA_LEN_1);
		ret = himax_bus_write(tmp_addr[0], NUM_NULL, tmp_data, 1);
		if (ret < 0) {
		...
		&& tmp_data2[0] == HX83121A_PARA_AHB_INC4)
			break;
...
int hx83121a_register_write(uint32_t addr, uint8_t *val, uint32_t len)
{
	int ret = -1;
	// uint8_t tmp_addr[DATA_LEN_4];
	...
	if (addr == ADDR_FLASH_SPI200_DATA)
		hx83121a_burst_enable(0);
	else if (len > DATA_LEN_4)
		hx83121a_burst_enable(1);
	else
		hx83121a_burst_enable(0);
...
static int hx83121a_register_read(uint32_t addr, uint8_t *buf, uint32_t len)
{
	int ret = -1;
	uint8_t tmp_data[DATA_LEN_4];
...
void hx83121a_system_reset(void)
{
	uint8_t tmp_data[DATA_LEN_4];

#if defined(HX_TP_TRIGGER_LCM_RST)
	hx_parse_assign_cmd(DATA_SYSTEM_RESET,
		tmp_data,
		DATA_LEN_4);
	hx83121a_register_write(ADDR_SYSTEM_RESET,
		tmp_data, DATA_LEN_4);
#else
	/* cmd reset */
	int ret;
	...
	hx83121a_interface_on();
	hx_parse_assign_cmd(DATA_CLEAR, tmp_data, DATA_LEN_4);
	hx83121a_register_write(ADDR_CTRL_FW,
		tmp_data, DATA_LEN_4);
	do {
		/* reset code*/
		/**
		...
		hx83121a_register_read(ADDR_FLAG_RESET_EVENT,
			tmp_data, DATA_LEN_4);
...
static bool hx83121a_sense_off(bool check_en)
{
	...
	do {
		...
		/* check fw status */
		hx83121a_register_read(ADDR_CS_CENTRAL_STATE,
			tmp_data, DATA_LEN_4);
		...
		hx83121a_register_read(ADDR_CTRL_FW, tmp_data, 4);
	...
	do {
		...
		if (tmp_data[0] == 0x0C) {
			...
			hx_parse_assign_cmd(DATA_CLEAR, tmp_data, DATA_LEN_4);
			hx83121a_register_write(ADDR_TCON_ON_RST,
				tmp_data, DATA_LEN_4);
			usleep_range(10000, 10001);
			ret = true;
			goto END;
...
#if defined(HX_ZERO_FLASH)
static void hx83121a_reload_to_active(void)
{
...
static void hx83121a_resume_ic_action(void)
{
#if !defined(HX_RESUME_HW_RESET)
	hx83121a_reload_to_active();
#endif
}
#endif
...
bool hx83121a_read_event_stack(uint8_t *buf, uint32_t length)
{
...
void hx83121a_init(void)
{
	hx_s_core_fp._interface_on = hx83121a_interface_on;
	...
	hx_s_core_fp._read_event_stack = hx83121a_read_event_stack;
...
#endif
	hx_s_core_fp._system_reset = hx83121a_system_reset;
	...
	hx_s_core_fp._burst_enable = hx83121a_burst_enable;
	hx_s_core_fp._register_read = hx83121a_register_read;
	hx_s_core_fp._register_write = hx83121a_register_write;

	hx_s_core_fp._sense_off = hx83121a_sense_off;
	hx_s_core_fp._sense_on = hx83121a_sense_on;
	hx_s_core_fp._chip_init = hx83121a_chip_init;

#if defined(HX_ZERO_FLASH)
	hx_s_core_fp._resume_ic_action = hx83121a_resume_ic_action;
	hx_s_core_fp._0f_reload_to_active = hx83121a_reload_to_active;
#endif
...
static bool hx83121a_chip_detect(void)
{
	uint8_t tmp_data[DATA_LEN_4];
	bool ret_data = false;
	int ret = 0;
	...
	if (hx83121a_sense_off(false) == false) {
		ret_data = false;
		E("%s:_sense_off Fail:\n", __func__);
		return ret_data;
	}
	for (i = 0; i < 5; i++) {
		ret = hx83121a_register_read(HX83121A_REG_ICID,
			tmp_data, DATA_LEN_4);
		...
		&& ((tmp_data[1] == 0x1a))) {
			strlcpy(hx_s_ts->chip_name,
				HX_83121A_SERIES_PWON, 30);
			(hx_s_ic_data)->ic_adc_num =
				HX83121A_DATA_ADC_NUM;
			hx_s_ic_data->isram_sz =
				HX83121A_ISRAM_SZ;
			hx_s_ic_data->dsram_sz =
				HX83121A_DSRAM_SZ;
			hx_s_ic_data->flash_size = HX83121A_FLASH_SIZE;
			hx_s_ic_setup = g_hx83121a_setup;
			hx83121a_init();
...
bool _hx83121a_init(void)
{
	bool ret = false;

	I("%s\n", __func__);
	ret = hx83121a_chip_detect();
	return ret;
}
