...
int himax_mcu_ulpm_in(void)
{
	...
	do {
		...
		chk = himax_bus_write(hx_s_ic_setup._addr_ulpm_34,
			NUM_NULL,
			tmp_write, 1);
		if (chk < 0) {
			I("%s: spi write fail!\n", __func__);
		...
		chk = himax_bus_read(hx_s_ic_setup._addr_ulpm_34, tmp_data, 1);
		if (chk < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
		}
	...
	do {
		...
		chk = himax_bus_read(hx_s_ic_setup._addr_ulpm_33, tmp_data, 1);
		if (chk < 0) {
			I("%s: spi read fail!\n", __func__);
			continue;
		}
	...
	do {
		...
		hx_parse_assign_cmd(hx_s_ic_setup._data_ulpm_aa,
			tmp_write, DATA_LEN_1);
		chk = himax_bus_write(hx_s_ic_setup._addr_ulpm_33,
			NUM_NULL,
			tmp_write, 1);
		if (chk < 0) {
			I("%s: spi write fail!\n", __func__);
...
bool himax_mcu_flash_programming(uint8_t *FW_content, int FW_Size)
{
	...
	page_prog_start += FLASH_RW_MAX_LEN) {
		...
		chk = hx_s_core_fp._register_write(
			hx_s_ic_setup._addr_flash_spi200_data,
