...
/* Add-start by tcl for otg voltage on 2023/08/08 */
#if defined(CONFIG_CHARGER_SC856X)
static struct charger_device *divider_charger;
#endif
/* Add-end by tcl for otg voltage on 2023/08/08 */
...
#if defined(CONFIG_CHARGER_SC856X)
	if (!divider_charger) {
		divider_charger = get_charger_by_name("primary_divider_chg");
		if (!divider_charger) {
			pr_info("%s: get divider charger device failed\n", __func__);
			return -ENODEV;
		}
	}
#endif
...
/* Add-start by tcl for otg voltage on 2023/08/08 */
#if defined(CONFIG_CHARGER_SC856X)
		charger_dev_enable_otg(divider_charger, true);
		msleep(10);
#endif
/* Add-end by tcl for otg voltage on 2023/08/08 */
...
#if defined(CONFIG_CHARGER_SC856X)
		msleep(10);
		charger_dev_enable_otg(divider_charger, false);
#endif
/* Add-end by tcl for otg voltage on 2023/08/08 */
		#if 0
			//# workaround
