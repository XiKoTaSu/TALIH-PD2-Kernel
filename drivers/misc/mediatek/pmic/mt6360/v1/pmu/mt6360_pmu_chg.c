...
/*Begin:add for resolve stand charger detect as none stand charger*/
static int mt6360_dpdm_init(struct mt6360_pmu_chg_info *mpci)
{
	int ret = 0;
	/* Begin set Dp&Dm to HZ status by wanglin.chen on 2021.08.21 */
	ret = mt6360_pmu_reg_update_bits(mpci->mpi,
					 MT6360_PMU_DPDM_CTRL,
					 0xff,
					 0x0);
	/* End set Dp&Dm to HZ status by wanglin.chen on 2021.08.21 */
	dev_err(mpci->dev, "[%s %d] ret=%d\n", __func__, __LINE__, ret);
	return ret;
}

/* Modify-start by baiwei.peng for usb if bc1.2 dcp test on 2022/12/19 */
#if !IS_ENABLED(TARGET_BUILD_USBIF_COMPLIANCE)
static int mt6360_dpdm_reset(struct mt6360_pmu_chg_info *mpci)
{
	int ret = 0;
	/* Begin Set Dp&Dm to 0v by wanglin.chen on 2021.08.21 */
	/* Set dpdm */
	ret = mt6360_pmu_reg_update_bits(mpci->mpi,
					MT6360_PMU_DPDM_CTRL,
					0xff,
					0xf3);
	/* End Set Dp&Dm to 0v by wanglin.chen on 2021.08.21 */
	dev_err(mpci->dev, "[%s %d] ret=%d\n", __func__, __LINE__, ret);
	return ret;
}
#endif
#endif
/* Modify-end by baiwei.peng for usb if bc1.2 dcp test on 2022/12/19 */
/*End:add for resolve stand charger detect as none stand charger*/
#ifdef CONFIG_MT6360_PMU_CHARGER_TYPE_DETECT
...
#if IS_ENABLED(CONFIG_TCT_CHARGER)
/*Begin:add for resolve stand charger detect as none stand charger by wanglin.chen*/
#if !IS_ENABLED(TARGET_BUILD_USBIF_COMPLIANCE)
	mt6360_dpdm_reset(mpci); //add by wanglin.chen on 2023.02.22
#endif
	mt6360_dpdm_init(mpci);
/*End:add for resolve stand charger detect as none stand charger by wanglin.chen*/
#endif
#ifdef CONFIG_TCPC_CLASS
