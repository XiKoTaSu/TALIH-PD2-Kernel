...
//wenhaodeng	
	if(conf == 2){
		data[3] = 0x22;
		printk("hhh,%s,data = 0x22\n",__func__);
	}else if(conf == 3){
		data[3] = 0x32;
		printk("hhh,%s,data = 0x32\n",__func__);
	}else if(conf == 4){
		data[3] = 0x42;
		printk("hhh,%s,data = 0x42\n",__func__);
	}else if(conf == 5){
		data[3] = 0x52;
		printk("hhh,%s,data = 0x52\n",__func__);
	}
...
static struct regulator *disp_vcn33_ldo;
static int regulator_inited = 0;
static int ldo33_regulator_init(void)
{
	int ret = 0;
	...
	disp_vcn33_ldo = regulator_get(NULL, "LDO1");   /**/
	if (IS_ERR(disp_vcn33_ldo)) { /* handle return value */
		ret = PTR_ERR(disp_vcn33_ldo);
		printk("get ldo_vcn33 fail, error: %d\n", ret);
		return ret;
	}
...
static int ldo33_enable(int en){
	int ret = 0;
	int retval = 0;
...
printk("deng, %s\n",__func__);	
	...
	ldo33_regulator_init();
	
#if 0	//----------------------
	ret = regulator_set_voltage(disp_vcn33_ldo, 3300000, 3300000);
	if (ret < 0)
		printk("deng, set voltage disp_vcn33_ldo fail, ret = %d\n", ret);
	retval |= ret;
#endif
	
	if(en ==1){
		ret = regulator_enable(disp_vcn33_ldo);
		if (ret < 0)
			printk("deng, enable regulator disp_vcn33_ldo fail, ret = %d\n",
				ret);
	}else{
		ret = regulator_disable(disp_vcn33_ldo);
		if (ret < 0)
			printk("deng, disable regulator disp_vcn33_ldo fail, ret = %d\n",
				ret);
	}
	retval |= ret;
	...
	unsigned long event, void *data)
{
	...
	if (event == TCP_NOTIFY_AMA_DP_STATE) {
		...
		if (!polarity) {
			...
			case 4:
			case 16:
			usbc_dbg(K_INFO, "set case 4/16 zc\n");
				pi3dpx1205a_set_eq_fg_sw(usbdp_client, 2);
				pi3dpx1205a_set_conf(usbdp_client, 2);   // byte 3 0x22 zc,  fc 0x32      byte4 0x06 
				break;
			case 8:
			case 32:
			usbc_dbg(K_INFO, "set case 8/32\n");
				pi3dpx1205a_set_eq_fg_sw(usbdp_client, 6);
				pi3dpx1205a_set_conf(usbdp_client, 6);
				break;
				usbc_dbg(K_INFO, "set case default\n");
			default:
			...
			case 4:
			case 16:
			usbc_dbg(K_INFO, "set case 4/16 fc\n");
...
//		}
		...
		usbc_dbg(K_INFO, "TCP_NOTIFY_TYPEC_STATE noti->typec_state.polarity:%x\n",noti->typec_state.polarity);		
		
		if (noti->typec_state.polarity == 0) {
			pi3dpx1205a_set_eq_fg_sw(usbdp_client, 4);
			pi3dpx1205a_set_conf(usbdp_client, 4);
			usbc_dbg(K_INFO, "TCP_NOTIFY_TYPEC_STATE:pi3dpx1205a_set_conf(usbdp_client, 4)zc\n");
		}else{
			pi3dpx1205a_set_eq_fg_sw(usbdp_client, 5);
			pi3dpx1205a_set_conf(usbdp_client, 5);
			usbc_dbg(K_INFO, "TCP_NOTIFY_TYPEC_STATE:pi3dpx1205a_set_conf(usbdp_client, 5)fc\n");
			}
