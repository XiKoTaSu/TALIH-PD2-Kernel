...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		...
		case ID_LIGHT:
			...
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->data[0];//lux
			event.word[1] = data->data[1];//Red
			event.word[2] = data->data[2];//Green
			event.word[3] = data->data[3];//Blue
			event.word[4] = data->data[4];//C
			event.word[5] = data->data[5];;//gain
			printk("xiakang.chen:%d,%d,%d,%d,%d,%d", event.word[0],event.word[1],event.word[2],event.word[3],event.word[4],event.word[5]);
			break;
		case ID_PROXIMITY:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
